...
export function Documents() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-10">
      <div>
        <h1 className="text-2xl font-bold text-stone-900 mb-1">Documents</h1>
        {course && <p className="text-sm text-stone-500 mb-6">{course.name}</p>}

        {error && <p className="mb-4 text-red-600 text-sm">{error}</p>}

        <form onSubmit={handleUpload} className="rounded-lg border border-stone-200 bg-white p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Week (optional)</label>
            <select
          ...
          <button
            type="submit"
            disabled={uploading}
            className="w-full rounded-lg bg-leaf-600 px-6 py-2.5 text-white font-medium hover:bg-leaf-700 disabled:opacity-50 transition-colors"
          >
        ...
        <div className="mt-6 space-y-3">
          {documents.length === 0 && <p className="text-sm text-stone-400">No documents uploaded yet.</p>}
          {documents.map((doc) => (
            <div
              key={doc.id}
            ...
            >
              <div>
                <p className="font-medium text-stone-900 text-sm">{doc.filename}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => handleDelete(doc.id)} className="text-xs text-stone-400 hover:text-red-600">
                  Delete
                </button>
              </div>
            </div>
      ...
      <div>
        <h2 className="text-lg font-semibold text-stone-900 mb-3">Search your material</h2>
        ...
        {results && (
          <div className="mt-4 space-y-3">
            {results.length === 0 && <p className="text-sm text-stone-400">No matches found.</p>}
            {results.map((r, i) => (
