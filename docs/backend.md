...
│   │   ├── vector_store.py      save_chunks, search_chunks (pgvector cosine distance)
...
`evaluation/`, `prompts/`) are intentionally not scaffolded — they belong to later phases and
would just be empty placeholders today. `rag/` and `ocr/` exist now (Phase 3) but only cover
ingestion — retrieval feeding an LLM for grounded answers is Phase 4.
